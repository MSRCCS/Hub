...
let resizeImg (imageData:byte[], maxWH:int, quality:int, resizeAlg:int)  =
    try
        use img = System.Drawing.Image.FromStream(new MemoryStream(imageData))
        if ( Math.Max(img.Width, img.Height) <= maxWH || maxWH = 0 ) && img.RawFormat.Guid = ImageFormat.Jpeg.Guid && img.PixelFormat = PixelFormat.Format24bppRgb then
            (img.Width, img.Height, imageData)
        else
            
            let interpolationMode:Drawing2D.InterpolationMode = enum resizeAlg
            ...
            let myEncoderPara = new EncoderParameter(myEncoder, int64 quality)
        ...
        with
            | _ as ex -> Logger.LogF( LogLevel.Error, fun _ -> sprintf "Exception %A when resizing input image to %d (quality:%d, interpolationAlog:%d). Emtpy array returned." ex maxWH quality resizeAlg )
                         (0, 0, [||])
...
let prepareImage imgFilePath imgSize quality interpolateAlgo=
    ...
    if (File.Exists(imgFilePath)) then
        ...
        let ( width, height, testImgBuf ) = resizeImg (imgBuf, imgSize, quality, interpolateAlgo)
...
let prepareImageCluster (name, imgBuf, imgSize, quality, interpolateAlgo) =
    // load the image and preprocess it: resize and convert to jpeg 85%
    let ext = Path.GetExtension( name )
    if String.Compare( ext, ".jpg", StringComparison.OrdinalIgnoreCase )=0 then 
            let testImgBuf = resizeImg (imgBuf, imgSize, quality, interpolateAlgo)
...
let textFileReader textFilePath = 
    seq {
        let fstream = new System.IO.StreamReader(textFilePath:string)
...
let loadImagesFromTSV (tsvFilePath:string, imgSize, quality, interpolateAlgo) =
    // load ground truth from TSV file (already randomly shuffled, each line: imagefilePath\tLabel\tBase64EncodedData)
    let GT =  textFileReader tsvFilePath
              |> Seq.map (fun line -> (line.Split([|'\t'|])))
              |> Seq.filter (fun items -> items.Length >= 3 )
    
    let imageBuf = GT |> Seq.map ( fun x -> let (imgFilePath, label, imgBuf ) = (x.[0].Trim(), x.[1].Trim().ToLower(), System.Convert.FromBase64String(x.[2]) )
                                            Logger.LogF( LogLevel.WildVerbose, ( fun _ -> sprintf "resizing %s..." imgFilePath))
                                            let ( width, height, testImgBuf ) = resizeImg (imgBuf, imgSize, quality, interpolateAlgo)
                                            Logger.LogF( LogLevel.WildVerbose, ( fun _ -> sprintf "resized image %s to %d X %d" imgFilePath width height ))
                                            imgFilePath, ( testImgBuf, imgBuf.Length )
...
// helper function: load image list in tsv files 
let loadImagesFromList (listFilePath:string, imgSize, quality, interpolateAlgo) =
    // load images from List text file, each line is a file path
    Logger.LogF( LogLevel.WildVerbose, ( fun _ -> sprintf "Loading image list file %s" listFilePath)) 
    let fileList =  textFileReader listFilePath
                    |> Seq.map (fun line -> line.Trim().ToLower())
                    |> Seq.filter (fun imgFilePath -> File.Exists(imgFilePath))
    
    let imageBuf = fileList |> Seq.map ( fun imgFilePath -> Logger.LogF( LogLevel.WildVerbose, ( fun _ -> sprintf "opening image input file %s" imgFilePath ))
                                                            let imgBuf = readBytesFromFile imgFilePath
                                                            Logger.LogF( LogLevel.WildVerbose, ( fun _ -> sprintf "resizing %s..." imgFilePath))
                                                            let ( width, height, testImgBuf ) = resizeImg (imgBuf, imgSize, quality, interpolateAlgo)
                                                            Logger.LogF( LogLevel.WildVerbose, ( fun _ -> sprintf "resized image %s to %d X %d" imgFilePath width height ))
...
let Usage = @"
    ...
    Command line arguments:
        -cmd            Recog     process a single file
                        RecogString process a text string
        ...
        -file           File_Path           file name or relative path of the input image which need to be recognized/tagged 
                                                required parameter for Recog command
        -inString       TextString          this string will be sent to the gateway and processed by the backend service
        -rootDir        Root_Directory      this directories holds all images 
        -TSV            File_Name           this TSV file hold all files to be processed, base64Encoded
        -fileList       File_Name           this fileList hold a list of files to be processed
        -svrListFile    File_Name           file path of the vHub gateway server list file, each line is a server
        -svrKey         #serverKey          field index of the server URL
        -svrListSplit   Characters          used to split the server list fields
        -resizeImage    NumOfPixelOfLongEdge resize the long edge of the image to NumOfPixelOfLongEdge, default 256, 0=no resize, use original image
        -jpgQuality     integer             1~100 for JPG compression quality
        -resizeAlg      biCubic/Linear      interpolation algorithm used to resize the image, default is 7: HighQualityBicubic
                                            0: Default, 1: Low, 2: High, 3: Bilinear, 4: Bicubic, 5: NearestNeighbor, 6: HighQualityBilinear, 7: HighQualityBicubic
        -checkResultFormat  true/false      whether check the recognition format : Tag1:Score1;...;TagN:ScoreN
    ...
    Example:
        ...
        CommandLineRecognition.exe -cmd Batch -Vhub vm-Hub.trafficManager.net -rootDir \\yuxiao-z840\onenet\data\images\office-bigset  -serviceGUID a0d6d212-58f5-cf2c-abbb-0713d32505bf  //office
        CommandLineRecognition.exe -cmd Batch -Vhub vm-Hub.trafficManager.net -rootDir \\yuxiao-z840\onenet\data\images\office-bigset  -serviceGUID f3dbd360-5e58-5765-1776-35a97ae29663  //VIA
    "
...
type WebInit(gateway, customerID, customerKey ) = 
    ...
    /// <param name = "aggrGuid">GUID of the result aggregator which will be used</param>
    /// <return>the response html string will be returned, including the information about inputs of the test</return>
    member x.Recognize ( imgFilePath:string, providerID, schemaID, serviceID, distID, aggrID , ?imgSize0, ?jpgQuality0, ?interpolationMode0) =
        WebInit.Initialized |> ignore
        let imgSize = defaultArg imgSize0 256
        let jpgQuality = defaultArg jpgQuality0 85
        let interpolationMode = defaultArg interpolationMode0 (int Drawing2D.InterpolationMode.HighQualityBicubic)
        let ( _, (imgBuf, oriSize)) = prepareImage imgFilePath imgSize jpgQuality interpolationMode
...
let main argv = 
    ...
    let tsvFilePath = parse.ParseString( "-TSV", "" ) // @".\combined.tsv"
    let listFilePath = parse.ParseString ( "-listFile", "" )
    let inString = parse.ParseString( "-inString", "" )
    ...
    let jpgQuality = parse.ParseInt("-jpgQuality", 85)
    let resizeAlg = parse.ParseInt("-resizeAlg", 7)
    let checkResultFormat = parse.ParseBoolean( "-checkResultFormat", true) 
    ...
    | "Recog" ->    
        ...
        for i = 0 to nRepeat - 1 do 
            let result = recog.Recognize(imgFilePath, providerID, schemaID, serviceID, distID, aggrID , resizeImage, jpgQuality, resizeAlg).Result
            Logger.LogF( LogLevel.Info, ( fun _ -> sprintf "Recognition result is \n%s" result ))
    | "RecogString" ->
        // to process text string
        let text = System.Text.Encoding.ASCII.GetBytes(inString)
        let recogResult = recog.RecognizeBuffer(text, providerID, schemaID, serviceID, distID, aggrID ).Result
        Logger.LogF( LogLevel.Info, ( fun _ -> sprintf "Recognition result is \n%s" recogResult ))
    | "List" ->
    ...
    | "Batch" ->
        ...
        let imgBufferBeforeFilter = if (File.Exists (tsvFilePath)) then 
                                        ...
                                        loadImagesFromTSV(tsvFilePath, resizeImage, jpgQuality, resizeAlg)
                                    elif(File.Exists (listFilePath)) then 
                                        // load the images from TSV file
                                        Logger.LogF( LogLevel.Info, ( fun _ -> sprintf "Loading images from %s ..." listFilePath))
                                        loadImagesFromList(listFilePath, resizeImage, jpgQuality, resizeAlg)
                                    else
                                        // load the images under imgRootDir
                                        ...
                                        let resizedImageBuffers = imgFiles |> Seq.map (fun x -> prepareImage x resizeImage jpgQuality resizeAlg)
                                        resizedImageBuffers
        ...
        for i = 0 to nRepeat - 1 do 
            for tuple in imgBuffer do
                ...
                let resultPairs = if (checkResultFormat) then parseRecogResult(recogResult) else [|("dummy",1.00)|]
        ...
        Logger.LogF( LogLevel.Info, ( fun _ -> sprintf "Processed %d Files with total %d Byte in %f sec, throughput = %f MB/s, avg recogtime = %d ms, %d files failed (%f\%%)" 
        ...
        for i = 0 to nRepeat - 1 do 
            let recogImageAndShow tuple = 
                async { 
                    ...
                    Logger.LogF( LogLevel.Info, ( fun _ -> sprintf "[Sent:%d Succ:%d Failed:%d / Total:%d] Image\t%s\tis recognized by \t%s\t as\n%s\n(resized to %dB, used %d ms)" 
                                                                    !numSentFiles !numSucceedFiles !numFailedFiles !numSentFiles name Guid recogResult recogSize recogMs))
                    }
