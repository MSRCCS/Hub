...
namespace VMHubClientLibrary
{
    ...
    public class GatewayHttpInterface 
    {
        ...
        private String curGateway = null; 
        /// <summary>
        ...
        { 
            ...
            set
            {
                if ( String.Compare( curGateway, value, StringComparison.OrdinalIgnoreCase )!=0 )
                {
                    curGateway = value; 
       ...
       public static byte[] EncodeToBytes<T>( T value)
        ...
        /// <summary>
        /// Check if the current provider is live on the gateway
        /// </summary>
        /// <returns>Liveness status</returns>
        public async Task<Boolean> CheckProviderStatus()
        {
            var bReturn = await this.CheckProviderStatus(GetCurrentProviderID());
            return bReturn; 
        }
        /// <summary>
        ...
        public async Task<String> ProcessAsyncString(Guid providerID, Guid schemaID, Guid domainID, Guid distributionID, Guid aggregationID, Byte[] buf)
        {
            ...
            try
            {
                ...
                catch (Exception ex)
                {
                    ...
                    var errorString = String.Format("Fails to decode the reply from current gateway {0} with request of {1}B, and reply of {2}B",
                        this.CurrentGateway,
                        buf.Length,
                        returnLength);
                    Debug.WriteLine(errorString);
            ...
            {               
                var errorString = String.Format("Fails to get a reply from current gateway {0} with request of {1}B", 
                    this.CurrentGateway, 
                    buf.Length);
                Debug.WriteLine(errorString);
                ErrorMessage = errorString;
